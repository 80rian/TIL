...
이제 괄호로 값 매개변수를, 그리고 독립된 함수 리터럴 블록으로 함수 매개변수를 전달함으로써 `safeStringOp` 호출이 더 깔끔해졌다. 또 다른 얘제로, 이름에 의한 매개변수 하나를 취하는 함수가 있다. 이 함수를 이름에 의한 매개변수 반환 타입과 메인 함수의 반환 타입을 위해 사용될 타입 매개변수를 이용하여 더 일반적으로 만들 것이다.
